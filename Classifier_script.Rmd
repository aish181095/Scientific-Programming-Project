...
sep_validation <- dummyVars(~ ., validation_classes, fullRank = FALSE)#seperating the class levels 
seperate_validation_class <- predict(sep_validation, validation_classes) %>% as.data.frame() #dataframe with separate classes 
```
...
```{r}
predict_reg_prob<-predict(opt_model_reg,validation_data,type="raw")


validation_reg<-data.frame(validation_data$class)
sep_validation_reg <- dummyVars(~ ., validation_reg, fullRank = FALSE)
seperate_validation_class_reg <- predict(sep_validation_reg, validation_reg) %>% as.data.frame()
```
...
plot(PRROC_reg_validation,main="ROC curve for Regression Ranger Model")
```
...
ggsave("Violin plot indicating the predicted regression values for Misclassified lables for validation data.png")
...
summary_covid %>%
  ...
  geom_violin()+ stat_summary(fun.data=mean_sdl, mult=2, 
             geom="pointrange", color="yellow")
#ggsave("Violin plot indicating the predicted regression values for True lables for validation data .png")
...
```{r}
predict_reg_test<-predict(opt_model_reg,test_data,type="raw")


test_reg<-data.frame(test_data$class)
sep_test_reg <- dummyVars(~ ., test_reg, fullRank = FALSE)
seperate_test_class_reg <- predict(sep_test_reg, test_reg) %>% as.data.frame()
```
...
plot(PRROC_reg_test,main="ROC curve for Test dataset for Regression Ranger Model")
```
```{r}
...
summary_covid_test %>%
  ...
  geom_violin()+ stat_summary(fun.data=mean_sdl, mult=2, 
             geom="pointrange", color="yellow")
ggsave("Violin plot indicating the predicted regression values for Misclassified lables for test data.png")
